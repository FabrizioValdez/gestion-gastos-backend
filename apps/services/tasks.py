...
def generar_notificaciones_vencimiento():
    ...
    for servicio in servicios:
        ...
        if dias_restantes == 3:
            ...
            if not ya_existe:
                nombre_servicio = servicio.nombre_servicio or servicio.catalogo_servicio.nombre
...
def generar_notificaciones_pago_pendiente():
    ...
    pagos_pendientes = Historial_pago.objects.filter(
        estado='pendiente'
    )
    ...
    for pago in pagos_pendientes:
        ...
        if not ya_existe:
            nombre_servicio = pago.servicio_usuario.nombre_servicio or pago.servicio_usuario.catalogo_servicio.nombre
