...
class Servicio_usuarioViewSet(viewsets.ModelViewSet):
    ...
    def por_tipo(self, request):
        ...
        servicios = self.get_queryset().filter(activo=True)
        por_tipo = (
            servicios.values("tipo_servicio__id", "tipo_servicio__nombre")
            .annotate(total=Sum("monto_mensual"), cantidad=Count("id"))
            .order_by("-total")
        )
    ...
    def graficos(self, request):
        ...
        pagos = Historial_pago.objects.filter(
            servicio_usuario__cliente=self.request.user, estado="pagado"
        )
        ...
        por_tipo = (
            pagos.values("servicio_usuario__tipo_servicio__nombre")
            .annotate(total=Sum("monto_pagado"))
            .order_by("-total")[:6]
        )
    ...
    def deudas(self, request):
        ...
        for servicio in servicios:
            ...
            nombre = (
                ...
                if servicio.catalogo_servicio
                else servicio.nombre_servicio
            )
...
class Historial_pagoViewSet(viewsets.ModelViewSet):
    ...
    def por_servicio(self, request, servicio_id=None):
        ...
        try:
            servicio = Servicio_usuario.objects.get(
                id=servicio_id, cliente=request.user
            )
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="reporte-pdf/(?P<anio>[0-9]+)/(?P<mes>[0-9]+)",
    )
    def reporte_pdf(self, request, anio=None, mes=None):
        ...
        try:
            ...
            return Response(
                {"error": "Año y mes inválidos"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for i, pago in enumerate(pagos, 1):
            nombre_servicio = (
                ...
                if pago.servicio_usuario.catalogo_servicio
            ...
            total += monto
            data.append([str(i), nombre_servicio[:30], fecha_pago, f"S/ {monto:.2f}"])
