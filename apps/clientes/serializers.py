...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        cliente = Cliente(
            nombre=validated_data["nombre"], correo=validated_data["correo"]
        )
...
class UpdateClienteSerializer(serializers.ModelSerializer):
    ...
    def validate_correo(self, value):
        if (
            Cliente.objects.exclude(pk=self.instance.pk)
            .filter(correo=value.lower())
            .exists()
        ):
    ...
    def validate(self, attrs):
        ...
        if password:
            if not password_actual:
                raise serializers.ValidationError(
                    {"password_actual": "Debe proporcionar la contraseña actual."}
                )
